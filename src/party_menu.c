...
static void CreatePartyMonEvolutionSprite(struct Pokemon *, struct PartyMenuBox *);
...
static void LoadPartyMenuBoxes(u8 layout)
{
    ...
    for (i = 0; i < PARTY_SIZE; i++)
    {
        ...
        sPartyMenuBoxes[i].evolutionSpriteId = SPRITE_NONE;
    }
...
static void RenderPartyMenuBox(u8 slot)
{
    ...
    UpdatePartyMonEvolutionSprites();
}
...
static void CreatePartyMonSprites(u8 slot)
{
    ...
    else if (GetMonData(&gPlayerParty[slot], MON_DATA_SPECIES) != SPECIES_NONE)
    {
        ...
        CreatePartyMonEvolutionSprite(&gPlayerParty[slot], &sPartyMenuBoxes[slot]);
    }
}
...
static void MovePartyMenuBoxSprites(struct PartyMenuBox *menuBox, s16 offset)
{
    ...
    gSprites[menuBox->evolutionSpriteId].x2 += offset * 8;
}
...
static void CursorCb_Evolution(u8 taskId)
{
    u16 evolution = GetEvolutionTargetSpecies(&gPlayerParty[gPartyMenu.slotId], EVO_MODE_NORMAL, ITEM_NONE, NULL);
...
static void CreatePartyMonEvolutionSprite(struct Pokemon *mon, struct PartyMenuBox *menuBox)
{
    if (GetMonData(mon, MON_DATA_SPECIES) != SPECIES_NONE)
    {
        menuBox->evolutionSpriteId = CreateSprite(&sSpriteTemplate_Evolution, menuBox->spriteCoords[8], menuBox->spriteCoords[9], 9);
        UpdatePartyMonEvolutionSprites();
    }
}
...
{
    LoadSpriteSheet(&sSpriteSheet_Evolution);
    LoadSpritePalette(&sSpritePalette_Evolution);
}